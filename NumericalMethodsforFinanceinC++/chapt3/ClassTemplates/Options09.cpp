...
/*Exercise 3.1 Replicating strategy*/
double EurOption::PriceByCRR(BinModel Model,
    ...
    int N = GetN();
    vector<double> Price(N+1);
    PortfolioTree.SetN(N-1);
    MoneyMarketTree.SetN(N-1);
    double x;
    double y;
    ...
    for (int n = N-1; n>=0; n--){
        for (int i=0; i<=n; i++){
            x = (Price[i+1]-Price[i])/(Model.S(n,i+1)-Model.S(n,i));
            PortfolioTree.SetNode(n,i,x);
            Price[i] = (q*Price[i+1]+(1-q)*Price[i])/(1+Model.GetR());
            y = (Price[i]-x*Model.S(n-1,i))/pow(1+Model.GetR(),n-1);
